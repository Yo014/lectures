...
//Author: Lauren Spargo 101345435
...
public class PathPlanner {
  ...
  private void runDijkstrasAlgorithmFrom(Node s) {
    ...
    //Initialize weight(v) of each vertex v to ∞ but initialize weight(s) of s to 0
    for(Node node : visibilityGraph.getNodes())
    {
    ...
    {
      v = queue.remove();
      edges = v.incidentEdges();

      for(int i = 0; i < edges.size(); i++)
      {
        u = edges.get(i).getEndNode();

        if(u.equals(v))
        {
          u = edges.get(i).getStartNode();
        }

        if(u.getDistance() > v.getDistance() + edges.get(i).length())
        {
          //remove for updating queue
          queue.remove(u);

          //Change u's weight, then put it back into the priority queue
          u.setDistance(v.getDistance() + edges.get(i).length());
          u.setPrevious(v);
          queue.add(u);
