...
// Author: Aslam Samy(SN:101361409)
...
public class Lab10Controller {
  ...
  static final double  MAX_SPEED      = 10;  // 19.1 is maximum but value of 1 is required for simulator to work properly for kinematics
  ...
  static void addSensorReadingsToMap(MapperApp mapper, Field translationField, double x, double y, double a) {
    ...
    for (int i = 0;i < 9; i++){
      ...
      double aRad= aOffsets[i];
      double xoff= xOffsets[i];
      double yoff= yOffsets[i];
      
      double xs= x + xoff * Math.cos(aRad)- yoff * Math.sin(aRad);
      double ys= y + yoff * Math.cos(aRad)+ xoff * Math.sin(aRad);
      
      double sensorAngleDeg= a+ aOffsets[i];
      double sensorAngleRad = Math.toRadians(sensorAngleDeg);
      
      double xobj= xs + d*Math.cos(sensorAngleRad);
      double yobj= ys + d*Math.sin(sensorAngleRad);
      
      mapper.addObjectPoint((int)xobj, (int)yobj);
